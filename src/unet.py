...
from .config import NUM_CHAN, NUM_ENCODER_LAYERS, NUM_INPUT_CHAN
...
class SpectralUNet(nn.Module):
    def __init__(self):
        ...
        self.class_conv = ConvLayer((NUM_ENCODER_LAYERS + 1) * NUM_CHAN, 1, kernel=[5, 5])
        self.class_pooling = nn.AdaptiveAvgPool2d(16)
    ...
    def forward(self, input_t):
        ...
        class_preds = self.class_pooling(self.class_conv(acts)).view(acts.size()[0], -1)
        classification_output = self.class_head(class_preds)
        ...
        return output_t, classification_output.view(-1)
